...
<head>
  ...
  <script src="../view/backoffice/assets/js/validation.js" defer></script>
  <script>
...
<body>
  ...
  <aside class="sidebar" id="sidebar" aria-label="Navigation administration">
    ...
    <nav class="sidebar-menu" aria-label="Menu principal">
      ...
      <a href="OffreController.php?action=admin_list" class="menu-item active"><span class="menu-item-icon" aria-hidden="true">&#8644;</span><span>Offres &amp; &eacute;changes</span></a>
          ...
          <section class="ssa-card" aria-labelledby="offres-admin-title" style="flex: 1.5;">
            <div class="ssa-card-header" style="flex-wrap: wrap; gap: 10px;">
              <div>
                ...
                <form method="get" action="OffreController.php" style="display:flex; gap:5px; margin-top:8px;">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td style="white-space: nowrap;">
                        <?php $next_etat = ($o['etat'] == 'bloqué') ? 'disponible' : 'bloqué'; ?>
                        <form method="post" action="OffreController.php?action=admin_toggle_block" style="display:inline;">
                            ...
                            <button type="submit" class="ssa-link" style="border:none; background:none; cursor:pointer; font-size: 0.82rem;">
                                <?= ($o['etat'] == 'bloqué') ? 'Débloquer' : 'Bloquer' ?>
                            </button>
                        </form> |
                        <form method="post" action="OffreController.php?action=admin_delete" style="display:inline;"><input type="hidden" name="id_offre" value="<?= $o['id_offre'] ?>"><button type="submit" class="ssa-link" style="border:none; background:none; cursor:pointer; color:red; font-size: 0.82rem;">Supprimer</button></form>
                    </td>
                  </tr>
                    ...
                    <td style="white-space: nowrap;">
                      <?php if($e['statut'] === 'en_attente'): ?>
                        <form method="post" action="EchangeController.php?action=traiter&source=admin" style="display:inline;">
                          ...
                          <input type="hidden" name="decision" value="accepté">
                          <button type="submit" class="ssa-link" style="border:none; background:none; cursor:pointer; font-size: 0.82rem;">Accepter</button>
                        </form> |
                        <form method="post" action="EchangeController.php?action=traiter&source=admin" style="display:inline;">
                          <input type="hidden" name="id_echange" value="<?= $e['id_echange'] ?>">
                          <input type="hidden" name="decision" value="refusé">
                          <button type="submit" class="ssa-link" style="border:none; background:none; cursor:pointer; font-size: 0.82rem;">Refuser</button>
                        </form> |
                      <?php endif; ?>
                      <?php $next_st = ($e['statut'] === 'bloqué') ? 'en_attente' : 'bloqué'; ?>
                      <form method="post" action="EchangeController.php?action=traiter&source=admin" style="display:inline;">
                          <input type="hidden" name="id_echange" value="<?= $e['id_echange'] ?>">
                          <input type="hidden" name="decision" value="<?= $next_st ?>">
                          <button type="submit" class="ssa-link" style="border:none; background:none; cursor:pointer; font-size: 0.82rem;">
                              <?= ($e['statut'] === 'bloqué') ? 'Débloquer' : 'Bloquer' ?>
                          </button>
                      </form> |
