...
class OffreC {
    ...
    public function ajouterOffre($obj) {
        try {
            $pdo = config::getConnexion();
            $sql = "INSERT INTO offreingredient (id_user, ingredient, categorie, quantite, unite_mesure, localisation, date_publication, etat, type_offre, description) 
    ...
    public function modifierOffre($id_offre, $quantite, $etat) {
        try {
            $pdo = config::getConnexion();
            $sql = "UPDATE offreingredient SET quantite = :quantite, etat = :etat WHERE id_offre = :id_offre";
            $req = $pdo->prepare($sql);
            return $req->execute(['quantite' => $quantite, 'etat' => $etat, 'id_offre' => $id_offre]);
        } catch (PDOException $e) { die('Erreur: ' . $e->getMessage()); }
    }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if ($action === 'add') {
                ...
                if (empty($ingredient) || empty($quantite) || empty($ville)) {
                    ...
                    $offre = new OffreIngredient($id_user, $ingredient, 'Autre', $quantite, 'kg', $ville);
                ...
                if ($id_offre && $nq) {
                    $this->modifierOffre($id_offre, $nq, 'disponible');
