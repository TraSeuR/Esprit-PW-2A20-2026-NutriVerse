...
class OrderController
{
    ...
    public function showForm()
    {
        ...
        require __DIR__ . '/../view/FrontOffice/commande/header.php';
        require __DIR__ . '/../view/FrontOffice/commande/commande.php';
        require __DIR__ . '/../view/FrontOffice/commande/footer.php';
    }
    ...
    public function validatePromo()
    {
        ...
        try {
            ...
            $stmt = $this->db->prepare("SELECT * FROM codes_promo WHERE code = ?");
            $stmt->execute([$code]);
    ...
    public function placeOrder()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            header('Location: shop.php?action=cart');
            exit();
        }
        ...
        if (!empty($codePromo)) {
            $stmtPromo = $this->db->prepare("SELECT * FROM codes_promo WHERE UPPER(code) = UPPER(?)");
        ...
        foreach ($cartItems as $item) {
            $stmtLine = $this->db->prepare("INSERT INTO ligne_commande (id_commande, id_produit, quantite, prix_unitaire) VALUES (?, ?, ?, ?)");
        ...
        if ($methodePaiement === 'carte') {
            ...
            $insertPaiement = $this->db->prepare("INSERT INTO paiement (commande_id, nom_carte, numero_carte, date_expiration, montant, statut, methode, date_paiement) VALUES (?, ?, ?, ?, ?, 'payé', 'carte bancaire', NOW())");
            $insertPaiement->execute([$orderId, $nomCarte, $numCarte, $dateExp, $total]);

            // 2. Mettre à jour la commande : 'expédiée' (en livraison)
            $this->db->prepare("UPDATE commande SET statut_commande = 'expédiée' WHERE id_commande = ?")->execute([$orderId]);
            ...
            header("Location: shop.php?action=order_confirmation&id=$orderId&payment=success");
            exit();
        }

        // Si paiement à la livraison, redirection normale
        header("Location: shop.php?action=order_confirmation&id=$orderId");
        exit();
    }
    ...
    public function confirmation()
    {
        ...
        require __DIR__ . '/../view/FrontOffice/commande/header.php';
        require __DIR__ . '/../view/FrontOffice/commande/confirmation.php';
        require __DIR__ . '/../view/FrontOffice/commande/footer.php';
    }
    ...
    public function myOrders()
    {
        ...
        require __DIR__ . '/../view/FrontOffice/commande/header.php';
        require __DIR__ . '/../view/FrontOffice/commande/mes_commandes.php';
        require __DIR__ . '/../view/FrontOffice/commande/footer.php';
    }
    ...
    public function orderDetail()
    {
        ...
        $stmtLivraison = $this->db->prepare("SELECT * FROM livraison WHERE id_commande = ?");
        $stmtLivraison->execute([$orderId]);
        $livraison = $stmtLivraison->fetch(PDO::FETCH_ASSOC);

        // On affiche la page des détails.
        require __DIR__ . '/../view/FrontOffice/commande/header.php';
        require __DIR__ . '/../view/FrontOffice/commande/commande_detail.php';
        require __DIR__ . '/../view/FrontOffice/commande/footer.php';
    }
    ...
    public function updateAddress()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $checkStmt = $this->db->prepare("SELECT statut_commande FROM commande WHERE id_commande = ?");
            $checkStmt->execute([$orderId]);
            $status = $checkStmt->fetchColumn();
            
            // Si c'est le cas, on met à jour l'adresse.
            if ($status === 'en attente') {
                $stmt = $this->db->prepare("UPDATE commande SET adresse_livraison = ? WHERE id_commande = ?");
                $stmt->execute([$newAddress, $orderId]);
            }
            
            // On retourne sur la page de détail de la commande.
            header('Location: shop.php?action=order_detail&id=' . $orderId);
            exit();
        }
    }
    ...
    public function cancelOrder()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($status === 'en attente') {
                $stmt = $this->db->prepare("UPDATE commande SET statut_commande = 'annulée' WHERE id_commande = ?");
                $stmt->execute([$orderId]);
            }
            
            // On retourne à la liste des commandes.
            header('Location: shop.php?action=my_orders');
            exit();
        }
    }
