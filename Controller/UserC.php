...
class userC
{
    ...
    public function processLogin()
    {
        ...
        $target = ($user['role'] !== 'utilisateur') ? "../../BackOffice/nutri_back.php" : "../index.php";

        if (isset($_SESSION['redirect_after_login'])) {
            // Prepend root path relative to the view/FrontOffice/utilisateur/ entry point
    ...
    public function processBackAddUser()
    {
        ...
        $email = trim(filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL));
        $mot_de_passe = $_POST['mot_de_passe'] ?? '';
        $allowed_roles = ['admin', 'utilisateur', 'Responsable recette', 'Responsable Produit', 'Responsable commande', 'Livreur', 'Responsable Programmes', 'Responsable Offre & ingrediant'];
        $role = in_array(trim($_POST['role'] ?? ''), $allowed_roles) ? trim($_POST['role']) : 'utilisateur';
        ...
        if (!empty($errors)) {
            ...
            // Called from BackOffice/utilisateur/ajoute.php → back.php is one level up
        ...
        header("Location: admin_utilisateurs.php?success=user_added");
        exit();
    }
    ...
    public function processBackUpdateUser()
    {
        ...
        $email = trim(filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL));
        $allowed_roles = ['admin', 'utilisateur', 'Responsable recette', 'Responsable Produit', 'Responsable commande', 'Livreur', 'Responsable Programmes', 'Responsable Offre & ingrediant'];
        $role = in_array($_POST['role'] ?? '', $allowed_roles) ? $_POST['role'] : 'utilisateur';
        ...
        if (!empty($errors)) {
            $_SESSION['back_errors'] = $errors;
            // Called from BackOffice/utilisateur/update.php → back.php is one level up
        ...
        header("Location: admin_utilisateurs.php?success=user_updated");
        exit();
    }
}
