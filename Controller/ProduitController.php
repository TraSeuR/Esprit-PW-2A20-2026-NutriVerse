...
class ProduitController {
    ...
    public function getDashboardStats(): array {
        ...
        try {
            $stats = [];
            $stats['total'] = $db->query("SELECT COUNT(*) FROM produit")->fetchColumn();
            $stats['actif'] = $db->query("SELECT COUNT(*) FROM produit WHERE statut='actif'")->fetchColumn();
            $stats['low_stock'] = $db->query("SELECT COUNT(*) FROM produit WHERE quantite_stock <= seuil_alerte")->fetchColumn();
            return $stats;
        } catch (Exception $e) {
            die("Erreur: " . $e->getMessage());
        }
    ...
    public function getLatestProduits(int $limit = 4): array {
        $db = config::getConnexion();
        try {
            $query = $db->prepare("SELECT * FROM produit WHERE statut='actif' ORDER BY idproduit DESC LIMIT :limit");
